...
class ThreadLSTM():
    ...
    def __init__(self):
        ...
        for i in range(100):
            self.target[0,i,0] = self.data[0,i,0]*((1-np.exp(-(i-30)/5.0)))
        ...
        for i in range(100):
            self.y_test[0,i,0] = self.x_test[0,i,0]*((1-np.exp(-(i-50)/5.0)))
    ...
    def train(self):
        for i in range(10):
        	with self.default_graph.as_default():
	            ...
	            self.model.fit(self.data, self.target, epochs=10, batch_size=1, verbose=2,validation_data=(self.x_test, self.y_test))
	            #predict = self.model.predict(self.x_test)
	            
	            predict = self.model.predict(self.data)
	            self.plot_data(self.x,[predict,self.target])

    def check_predict(self,inp,exp_out):
    ...
    def test(self):
        while True:
            ...
            self.check_predict(self.x_test,self.y_test)
            time.sleep(0.1)
