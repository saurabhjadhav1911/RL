#C:\Users\saurabhj\OneDrive\Documents\Python Scripts
import numpy as np
import random
import cv2
N=0
...
class RLMaze():
    ...
    def Epoch(self):
        ...
        while(self.exit==1):
            self.prpos=self.pos
            print('assign',self.prpos)
            n+=1
            act=self.choose_action(self.pos)
            print('chact',self.prpos)
            Q=self.getQ([self.prpos[0],self.prpos[1]],act)
            print('aftQ',self.prpos)
            self.exit,pos,self.move_reward=self.Move(self.pos,act)
            print('aftmove',self.prpos)
            self.total_reward+=self.move_reward
            print('end',self.prpos,self.pos)
            #print("move_reward",r)
            self.render()
            ...
            self.putQ([self.prpos[0],self.prpos[1]],act,Q)
            self.pos=pos
